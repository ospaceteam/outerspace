...
class IUniverse(IObject):
    ...
    def createNewSubscribedGalaxy(self, tran, obj, galaxyName, galaxyType, listOfPlayers):
        ...
        galaxy_stats = GalaxyGenerator.GalaxyStats(galaxyType)
        pos_x, pos_y = self.cmd(obj).findSpotForGalaxy(tran, obj, galaxy_stats.galaxyRadius)
        log.message("Adding new galaxy '%s' to (%d, %d)" % (galaxyType, pos_x, pos_y))
        ...
        if newGalaxy.scenario == SCENARIO_SINGLE:
            ...
            # let's name them after the player login
            try:
                lonePlayerLogin = listOfPlayers[0]
                newGalaxy.name = "Single - %s" % (lonePlayerLogin,)
            except IndexError:
                # empty list of players - galaxy generated by command
                # thus we do not want to alter galaxy name
                pass
