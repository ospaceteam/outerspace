	...
	def FillValues(self):
		if not self.selectedDesign.empty:
			if self.selectedDesign.hullName != None:
				self.hullBtn.SetTitle(self.selectedDesign.hullName)
			else:
				self.hullBtn.SetTitle("[Click to select]")
			if self.selectedDesign.cockpitName != None:
				self.controlBtn.SetTitle(self.selectedDesign.cockpitName)
			else:
				self.controlBtn.SetTitle("[Click to select]")
			self.lblClass.SetTitle(sequip.cclass2Text(self.selectedDesign.combatClass))
			self.lblSignature.SetTitle("%d" % self.selectedDesign.signature)
			self.lblSpeed.SetTitle("%.2f" % self.selectedDesign.speed)
			if self.selectedDesign.shieldHP > 0:
				self.lblHP.SetTitle("%d - %d" % (self.selectedDesign.maxHP, self.selectedDesign.shieldHP))
			else:
				self.lblHP.SetTitle("%d" % self.selectedDesign.maxHP)
			self.lblAttack.SetTitle("%d" % self.selectedDesign.combatAtt)
			self.lblDefence.SetTitle("%d / %d" % (self.selectedDesign.combatDef, self.selectedDesign.missileDef))
			self.lblPayload.SetTitle("%d" % self.selectedDesign.payload)
			self.lblFreeSlots.SetTitle("%d" % self.selectedDesign.freeSlots)
			self.lblTanks.SetTitle("%d" % self.selectedDesign.storEn)
			self.lblSuppFuel.SetTitle("%d" % self.selectedDesign.operEn)
			if self.selectedDesign.speed > 0:
				...
				self.lblRange.SetTitle("%.2f" % (support * self.selectedDesign.speed / Rules.turnsPerDay))
			else:
				self.lblRange.SetTitle("none")
			
			self.lblConstrPts.SetTitle("%d" % self.selectedDesign.buildProd)
			self.lblSuppConstr.SetTitle("%d" % (self.selectedDesign.buildProd * Rules.operProdRatio))
			self.lblBaseExp.SetTitle("%d" % self.selectedDesign.baseExp)
			self.lblMP.SetTitle("%d" % self.selectedDesign.combatPwr)
			self.designName.SetTitle(self.selectedDesign.name)
			i = 0
			...
			self.controlBtn.SetTitle("[Click to select]")
			na = "N/A"
			self.lblClass.SetTitle(na)
			self.lblSignature.SetTitle(na)
			self.lblSpeed.SetTitle(na)
			self.lblHP.SetTitle(na)
			self.lblAttack.SetTitle(na)
			self.lblDefence.SetTitle(na)
			self.lblPayload.SetTitle(na)
			self.lblFreeSlots.SetTitle(na)
			self.lblTanks.SetTitle(na)
			self.lblSuppFuel.SetTitle(na)
			self.lblRange.SetTitle(na)
			self.lblConstrPts.SetTitle(na)
			self.lblSuppConstr.SetTitle(na)
			self.lblBaseExp.SetTitle(na)
			self.lblMP.SetTitle(na)
			self.designName.SetTitle("")
